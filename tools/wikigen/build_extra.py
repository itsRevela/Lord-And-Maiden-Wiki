...
def gen_knowledge_quiz(write, tbl, R):
    lines = [
        ...
        "## Why there's no question list here",
        "Unlike stats, items and costs — which live in the game's config files and are fully data-mined "
        "across this wiki — the quiz's **questions, answer options and correct answers are delivered by "
        "the game server at runtime**. Each question's text, its options and the correct-answer flag are "
        "read from the network, not stored in the client, so a question bank **cannot be extracted from "
        "the game files** (it would require capturing live server data, which is outside this project's "
        "scope). This page therefore documents how the quiz works rather than listing questions.", "",
